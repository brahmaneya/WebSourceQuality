...
# clean logging file
commands.getstatusoutput('rm stock_experiments.log')

# Initialize logging
logger = logging.getLogger("exps")
logger.setLevel(logging.DEBUG)

fh = logging.FileHandler(os.path.join(os.path.dirname(os.path.realpath("__file__")), "stock_experiments.log"),"w")
...
for h in holdoutSizes:
	...
	# Set holdout set
	commands.getstatusoutput('sed -i \'s/calibration.holdout_fraction: [0-9].[0-9]/calibration.holdout_fraction: '+str(h)+'/g\' sources_only.conf')
	# init dd
...
for h in holdoutSizes:
	...
	# Set holdout set
	commands.getstatusoutput('sed -i \'s/calibration.holdout_fraction: [0-9].[0-9]/calibration.holdout_fraction: '+str(h)+'/g\' sources_w_features.conf')
