...
fh = logging.FileHandler(os.path.join(os.path.dirname(os.path.realpath("__file__")), "event_experiments_gibbs_perf_feats.log"),"w")
...
for h in holdoutSizes:
 	...
 	# Set holdout set
	commands.getstatusoutput('sed -i \'s/calibration.holdout_fraction: [0-9].[0-9]\+/calibration.holdout_fraction: '+h+'/g\' confsForExps/sources_only_gibbs.conf')
 	...
 	commands.getstatusoutput('deepdive run -c confsForExps/sources_only_gibbs.conf')
...
for h in holdoutSizes:
	...
	# Set holdout set
	commands.getstatusoutput('sed -i \'s/calibration.holdout_fraction: [0-9].[0-9]\+/calibration.holdout_fraction: '+h+'/g\' confsForExps/sources_w_features_gibbs.conf')
	...
	start = timeit.default_timer()
	commands.getstatusoutput('deepdive run -c confsForExps/sources_w_features_gibbs.conf')
	stop = timeit.default_timer()
...
# backup 
commands.getstatusoutput('cp event_experiments_gibbs_perf_feats.log expResults/.')
