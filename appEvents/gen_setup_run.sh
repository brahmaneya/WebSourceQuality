...
deepdive sql "select bucket, is_true, count(*) from event_truth_is_true_inference_bucketed where is_true IS NOT NULL group by bucket, is_true order by bucket, is_true"
deepdive sql "select 'True', is_true, count(*) from event_truth_is_true_inference_bucketed where is_true IS NOT NULL and bucket < 5 group by is_true order by is_true"
deepdive sql "select 'True', is_true, count(*) from event_truth_is_true_inference_bucketed where is_true IS NOT NULL and bucket >= 5 group by is_true order by is_true"
echo 'Max based evaluation for positive class'
deepdive sql "select 'True', tt.is_true, count(*) from event_truth_is_true_inference_bucketed tt, (select geid, max(expectation) as mex from event_truth_is_true_inference_bucketed where is_true is not null group by geid) as groupedtt where tt.geid = groupedtt.geid and tt.expectation = groupedtt.mex and tt.expectation > 0 group by tt.is_true"
