...
fh = logging.FileHandler(os.path.join(os.path.dirname(os.path.realpath("__file__")), "event_experiments_gibbs_perf_em.log"),"w")
...
for h in holdoutSizes:
 	...
 	# Set holdout set
	commands.getstatusoutput('sed -i \'s/calibration.holdout_fraction: [0-9].[0-9]\+/calibration.holdout_fraction: '+h+'/g\' confsForExps/sources_only_em.conf')
...
for h in holdoutSizes:
	...
	# Set holdout set
	commands.getstatusoutput('sed -i \'s/calibration.holdout_fraction: [0-9].[0-9]\+/calibration.holdout_fraction: '+h+'/g\' confsForExps/sources_w_features_em.conf')
...
# backup 
commands.getstatusoutput('cp event_experiments_gibbs_perf_em.log expResults/.')
