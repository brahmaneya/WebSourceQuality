...
fh = logging.FileHandler(os.path.join(os.path.dirname(os.path.realpath("__file__")), "stock_experiments_em_perf_lowHO.log"),"w")
...
for h in holdoutSizes:
 	...
 	# Set holdout set
	commands.getstatusoutput('sed -i \'s/calibration.holdout_fraction: [0-9].[0-9]/calibration.holdout_fraction: '+str(h)+'/g\' confsForExps/sources_only_em.conf')
 	...
 	commands.getstatusoutput('deepdive run -c confsForExps/sources_only_em.conf')
...
for h in holdoutSizes:
	...
	# Set holdout set
	commands.getstatusoutput('sed -i \'s/calibration.holdout_fraction: [0-9].[0-9]/calibration.holdout_fraction: '+str(h)+'/g\' confsForExps/sources_w_features_em.conf')
	...
	start = timeit.default_timer()
	commands.getstatusoutput('deepdive run -c confsForExps/sources_w_features_em.conf')
	stop = timeit.default_timer()
...
# backup 
commands.getstatusoutput('cp stock_experiments_em_perf_lowHO.log expResults/.')
