...
% add prior terms for S_1, G_1 etc if not using uniform prior.
\begin{align*} 
...
\\& (\prod_{j=1}^{n} T_{j}^{T}T \prod_{k} T_{j, k}^{T}G_k \prod_{i:s_i\in g_k} T_{j,k,i}^{T}S_i) \times 
\\& (\prod_{j} \prod_{i} \epsilon^{I(b_{i,j} = \lor_{k : s_i \in g_k} T_{j,k,i})})
\\& (\prod_{l} \prod_{1 \leq j_1 < j_2 \leq n} \epsilon^{I(T_{j_1} \land T_{j_2} \land (t_{j_1}\in H_l) \land (t_{j_2} \in H_l)}))
\end{align*}
where $I$ is an indicator function, and $P(T), P(G_k), P(S_i)$ are the prior probability densities. 
