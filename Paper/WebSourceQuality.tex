...
%write:
% existing work
% why our setting is more general. our design choices. prior, etc. see draft. 
% our setting (already written). change the deepdive part, write about our naive pseudo code
% problems with setting. sparsity
% suggestions for improvment
% several choices/assumptions for improvement. generalizing jiawei han's work. 
...
% problem with model. large number of false tuples, sources. so large number of boolean variables (so memory). during gibbs sampling, have to iterate over many variables. also for each variable, have to check a large number of probabilities (as the flip prob is this big product). and then if change of variable, have to update a lot of counts. all of these are at least |T||S| time. have to do this for 10000 iterations per sample(?). 
% solution. lots of tuples false. even for true tuples, source-output graph (b_{i,j}) is sparse. so instead of maintaining all booleans, only maintain those likely to be true. i.e. join of ST and SG. How to deal with combining probabilities (for both T's and GT's) and updating counts. 
